...
async function combineElements(
  ...
  const response = await fetch(
    "https://sv-activation.openai.azure.com/openai/deployments/sv-activation/chat/completions?api-version=2024-02-15-preview",
    {
      method: "POST",
      headers: {
  ...
  const sanitazed = data.choices[0].message.content
    .replace(/```json\n/g, "")
    .replace(/\n```/g, "");
...
async function checkForExistingCombination(
  elementA: string,
  elementB: string
): Promise<IElement | null> {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsedBody = validateRequestBody<z.infer<typeof ElementDTO>>(
      ElementDTO,
      body
    );
    ...
    if (!activation) {
      return NextResponse.json(
        { message: "Activation Doesn't exists " },
        { status: 400 }
      );
    }
