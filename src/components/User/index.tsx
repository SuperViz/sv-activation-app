...
export function TVUser({ user }: { user: IUser }) {
  ...
  const userHasStar =
    activationsNumber === Object.keys(ActivationColor).length &&
    user.activations.every(
      (activation) => activation.completed || (activation.quantity || 0) >= 10
    );

  return (
    <div
      className={`flex flex-col items-center justify-center ${
        user.isOnline ? "opacity-100" : "opacity-20"
      }`}
    >
      ...
      <div className={`relative w-[11.5rem] h-[11.5rem]`}>
        ...
        <div className="baseUser">
          <span className="text-[#26242A] text-lg tv:text-[2.25rem] font-black">
            {firstLetter.toUpperCase()}
          </span>
        </div>
      </div>
      <p className="text-[1rem] tv:text-[2rem] mt-1.5 max-w-[12.125rem] break-words text-center">
        {user.name}
      </p>
    </div>
  );
...
export function MobileUser({ user }: { user: IUser }) {
  ...
  const userHasStar =
    activationsNumber === Object.keys(ActivationColor).length &&
    user.activations.every((activation) => activation.completed);
  ...
  return (
    <div className={`flex flex-col items-center justify-center`}>
      ...
      <p className="text-[1rem] mt-1.5 max-w-[6.5rem] break-words text-center">
        {user.name}
      </p>
    </div>
  );
